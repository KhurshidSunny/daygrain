...
function removeBadParentLink() {
  if (existsSync(badLink)) {
    ...
    console.log('Removed tools/focus/node_modules/daygrain-hub symlink')
  }
}
...
// Editor/npm races may re-inject the parent package — strip only that
...
if (!existsSync(join(modulesDir, 'vite', 'package.json'))) {
  console.error('vite was not installed into tools/focus/node_modules')
  process.exit(1)
}

if (existsSync(badLink)) {
  console.error('Parent symlink still present')
  process.exit(1)
}

console.log('Focus deps installed cleanly.')
